...
public class UPGrowthMainAlgorithms {

	int threshold;
    //fp-tree constructing fileds
    Vector<UPtree_table> headerTable;
    UPtree_table fptree;
    //fp-growth
    Map<String, Integer> frequentPatterns;

    public UPGrowthMainAlgorithms(File file, int threshold) throws FileNotFoundException {
        this.threshold = threshold;
        fptree(file);
     /*   Upgrowth(fptree, threshold, headerTable);*/
        print();

    }

    private UPtree_table conditional_fptree_constructor(Map<String, Integer> conditionalPatternBase, Map<String, Integer> conditionalItemsMaptoFrequencies, int threshold, Vector<UPtree_table> conditional_headerTable) {
        //FPTree constructing
        //the null node!
    	UPtree_table conditional_fptree = new UPtree_table("null");
        ...
        for (String pattern : conditionalPatternBase.keySet()) {
            ...
            StringTokenizer tokenizer = new StringTokenizer(pattern); // CATEGORY_bLUELABEL
    ...
    private void construct_fpTree(File file, Map<String, Integer> itemsMaptoFrequencies, Scanner input, List<String> sortedItemsbyFrequencies, Vector<String> itemstoRemove) throws FileNotFoundException {
        ...
        headerTable = new Vector<UPtree_table>();
        for (String itemsforTable : sortedItemsbyFrequencies) {
            headerTable.add(new UPtree_table(itemsforTable));
        }
        //FPTree constructing
        input = new Scanner(file);
        //the null node!
        fptree = new UPtree_table("null");
        fptree.item = null;
        fptree.root = true;
        ...
        for (UPtree_table item : headerTable) {
            int count = 0;
            UPtree_table itemtemp = item;
    ...
    void insert(Vector<String> transactionSortedbyFrequencies, UPtree_table fptree, Vector<UPtree_table> headerTable) {
        if (transactionSortedbyFrequencies.isEmpty()) {
            return;
        }
        String itemtoAddtotree = transactionSortedbyFrequencies.firstElement();
        UPtree_table newNode = null;
        boolean ifisdone = false;
        for (UPtree_table child : fptree.children) {
        ...
        if (!ifisdone) {
            newNode = new UPtree_table(itemtoAddtotree);
            newNode.count = 1;
            newNode.parent = fptree;
            fptree.children.add(newNode);
            for (UPtree_table headerPointer : headerTable) {
    ...
    private void fpgrowth(UPtree_table fptree, int threshold, Vector<UPtree_table> headerTable) {
        frequentPatterns = new HashMap<String, Integer>();
        FPgrowth(fptree, null, threshold, headerTable, frequentPatterns);
        int i = 0;
    }

    void FPgrowth(UPtree_table fptree, String base, int threshold, Vector<UPtree_table> headerTable, Map<String, Integer> frequentPatterns) {
        for (UPtree_table iteminTree : headerTable) {
            ...
            while (iteminTree.next != null) {
                ...
                UPtree_table conditionalItem = iteminTree.parent;
            ...
            Vector<UPtree_table> conditional_headerTable = new Vector<UPtree_table>();
            for (String itemsforTable : conditionalItemsMaptoFrequencies.keySet()) {
                ...
                UPtree_table f = new UPtree_table(itemsforTable);
                f.count = count;
                conditional_headerTable.add(f);
            }
           UPtree_table conditional_fptree = conditional_fptree_constructor(conditionalPatternBase, conditionalItemsMaptoFrequencies, threshold, conditional_headerTable);
            //headertable reverse ordering
    ...
    private void insert(Vector<String> pattern_vector, int count_of_pattern, UPtree_table conditional_fptree, Vector<UPtree_table> conditional_headerTable) {
        ...
        UPtree_table newNode = null;
        boolean ifisdone = false;
        for (UPtree_table child : conditional_fptree.children) {
            if (child.item.equals(itemtoAddtotree)) {
                newNode = child;
        ...
        if (!ifisdone) {
            for (UPtree_table headerPointer : conditional_headerTable) {
