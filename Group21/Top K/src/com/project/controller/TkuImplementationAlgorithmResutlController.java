...
/**
 * Servlet implementation class TkuImplementationAlgorithmResutlController
 */
@WebServlet("/TkuImplementationAlgorithmResutlController")
public class TkuImplementationAlgorithmResutlController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TkuImplementationAlgorithmResutlController() {
        super();
		...
		sessionF.setAttribute("finalTime1Session",timeFinal1);
				...
				try {
					...
					if(userSearchBookBeans.getUser_paramertre().equals("Hit")){
						...
						int n=userDao.userTkoResultGraph(userSearchBookBeans.getUser_category(), userSearchBookBeans.getUser_top_k_value(), totalTko);
					...
					while (rs2.next()) {
						...
						System.out.println("Product Is is ............ "
								+ rs2.getInt(2));

						UserProductBeans userProductBeans = new UserProductBeans();

						rsF = userDao.userGetInfomationFinal(rs2.getInt(2));

						// id, product_id, brand_name, product_name, SKU, SRP,
						// gross_weirth, net_weigth, recylared, low_fat,
						// units_pre, Case_pre, shelf_heigth, shelf_width,
						// shelf_depth, product_hit, product_Buy

						while (rsF.next()) {

							// id, product_id, product_name, product_Brand,
							// product_SKU, product_SRP, gross_weight,
							// net_weight, recyclable_package, low_fat,
							// units_per_case, cases_per_pallet, shelf_width,
							// shelf_height, shelf_depth, product_price
							...
							userProductBeans.setRecyclable_package(rsF
									.getString(9));

							userProductBeans.setLow_fat(rsF.getString(10));

							userProductBeans.setUnits_per_case(rsF
									.getString(11));

							userProductBeans.setCases_per_pallet(rsF
									.getString(12));
							...
							userProductBeans
									.setProduct_price(rsF.getString(16));
